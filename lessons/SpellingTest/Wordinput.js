...
function getNewPrompt() { //gets a random word and displays the defenition
    ...
    document.getElementById('prompt').textContent = "Definition: " + randomWord.def; //set the prompt to the defenition of that word
...
document.getElementById('testForm').addEventListener('submit', function(event) { //runs when the user submits their typed word for grading
    ...
    if (userAnswer === correctWord) { //if it's the same word
        ...
        correctAnswers++; //increase correct anwsers by 1
        ...
        document.getElementById('feedback').textContent = 'Oops! That’s not correct. Try again.'; //show the faliure text
